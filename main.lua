-- NURYSIUM UI (BLUE THEME) - FULL SCRIPT
local nurysium = {}
...
local is_open = false
local BLUE = { primary = Color3.fromRGB(66,133,244), dark = Color3.fromRGB(30,80,180), text = Color3.fromRGB(231,231,243) }
local function animate_elements(speed)
    ...
    tween_service:Create(layout, TweenInfo.new(speed, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Padding = UDim.new(0.02, 0) }):Play()
end
function nurysium: open()
    ...
    local layout = ui.Background["functions_frame"]:FindFirstChild("UIListLayout")
    if layout then tween_service:Create(layout, TweenInfo.new(2, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Padding = UDim.new(0.02, 0) }):Play() end
    tween_service:Create(ui.Background.Title, TweenInfo.new(1.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { TextTransparency = 0 }):Play()
    tween_service:Create(ui.Background, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Size = UDim2.new(0, 655, 0, 325), Position = UDim2.new(0.4, 0, 0.3, 0), BackgroundTransparency = 0 }):Play()
end
function nurysium: close()
    ...
    local layout = ui.Background["functions_frame"]:FindFirstChild("UIListLayout")
    if layout then tween_service:Create(layout, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Padding = UDim.new(0.02, 0) }):Play() end
    tween_service:Create(ui.Background.Title, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { TextTransparency = 1 }):Play()
    tween_service:Create(ui.Background, TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0.510, 0, 1, 0), BackgroundTransparency = 1 }):Play()
end
function nurysium: toggle() if is_open then self:close() else self:open() end end
function nurysium: init(name, parent)
    if parent:FindFirstChild(name) then parent:FindFirstChild(name):Destroy() end
    ...
    TitleGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, BLUE.primary), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(130,177,255))}
    ...
    logo.MouseButton1Up:Connect(function() nurysium:toggle() end)
    logo.TouchTap:Connect(function() nurysium:toggle() end)
    task.defer(function()
...
function nurysium: create_toggle(name, section_name, callback)
    ...
    local function toggle_action()
        ...
        callback(toggled)
        if toggled then
            tween_service:Create(Dot, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Position = UDim2.new(0.600,0,0.224,0), BackgroundColor3 = BLUE.primary }):Play()
            tween_service:Create(UIStroke_2, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Transparency = 0.5, Color = BLUE.primary }):Play()
            tween_service:Create(Toggle, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { BackgroundColor3 = BLUE.dark }):Play()
        else
            tween_service:Create(Dot, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Position = UDim2.new(0.149,0,0.224,0), BackgroundColor3 = Color3.fromRGB(37,35,48) }):Play()
            tween_service:Create(UIStroke_2, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { Transparency = 0, Color = Color3.fromRGB(40,39,45) }):Play()
            tween_service:Create(Toggle, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), { BackgroundColor3 = Color3.fromRGB(27,24,35) }):Play()
        end
    end
...
function nurysium: create_button(name, section_name, callback)
    ...
    UIGradient.Rotation = 36
    UIGradient.Parent = UIStroke
    btn.MouseEnter:Connect(function() tween_service:Create(btn, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { BackgroundColor3 = Color3.fromRGB(45,42,55) }):Play() end)
    btn.MouseLeave:Connect(function() tween_service:Create(btn, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { BackgroundColor3 = Color3.fromRGB(255,255,255) }):Play() end)
...
function nurysium: create_slider(name, section_name, min, max, default, callback)
    ...
    Hitbox.TouchTap:Connect(function() update_slider(user_input:GetMouseLocation()) end)
    user_input.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then is_dragging = false end end)
    user_input.InputChanged:Connect(function(input) if is_dragging and input.UserInputType == Enum.UserInputType.MouseMovement then update_slider(input.Position) end end)
    run_service.Heartbeat:Connect(function()
        if not section_name:match(ui_data.current_section) and not table.find(search_table, name) then
            frame.Visible = false
        else
...
function nurysium: MakeNotify(data)
    ...
    contL.Parent = notif
    tween_service:Create(notif, TweenInfo.new(0.3, Enum.EasingStyle.Quad), { Position = UDim2.new(1,-320,0,20) }):Play()
    task.wait(3)
    tween_service:Create(notif, TweenInfo.new(0.5, Enum.EasingStyle.Quad), { Position = UDim2.new(1,-320,0,-100), BackgroundTransparency = 1 }):Play()
    task.wait(0.5)
    notif:Destroy()
end
return nurysium
...
local state = { autoJump = false, fly = false, noclip = false, esp = false, jumpConn = nil }
    ...
    if toggled then
        if not flyButtons then
            ...
            local function setupBtn(btn, dir)
                btn.MouseButton1Down:Connect(function() flyDir = flyDir + dir updateFly() end)
                btn.MouseButton1Up:Connect(function() flyDir = flyDir - dir updateFly() end)
                btn.TouchBegan:Connect(function() flyDir = flyDir + dir updateFly() end)
                btn.TouchEnded:Connect(function() flyDir = flyDir - dir updateFly() end)
            end
